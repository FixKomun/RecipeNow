    ...
    <b-container>
      <b-row class="content">
        <b-col class="left" cols="7">
          ...
          <Button @click="login" :loading="isLogging" :disabled="isLogging">
            Log in
            <svg
              width="28"
              height="28"
...
export default {
  ...
  methods: {
    async login() {
      this.isLogging = true;
      const res = await this.callApi("post", "/app/login", this.userData);
      if (res.status === 200) {
